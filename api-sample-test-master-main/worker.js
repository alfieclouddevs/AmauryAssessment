      ...
      (
        await (
          await hubspotClient.apiRequest({
            ...
            body: {
              inputs: contactsToAssociate.map(contactId => ({ id: contactId })),
            },
          })
        ).json()
      )?.results || [];
  ...
  while (hasMore) {
    ...
    await Promise.all(
      data.map(async meeting => {
        ...
        const res =
          await hubspotClient.crm.objects.meetings.associationsApi.getAll(
            meeting.id,
            "contacts"
          );
        ...
        const actionTemplate = {
          includeInAnalytics: 0,
          meetingProperties: {
            meeting_title: meeting.properties.hs_meeting_title ?? "No title",
            created_at: new Date(meeting.properties.hs_createdate),
            updated_at: new Date(meeting.properties.hs_lastmodifieddate),
          },
        ...
        const isCreated =
          new Date(meeting.properties.hs_createdate) > lastPulledDate;
